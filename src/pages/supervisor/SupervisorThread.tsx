...
import { staffMessageService, type MessageAttachment } from "@/lib/services";
...
import styles from "./SupervisorThread.module.css";
...
export default function SupervisorThread() {
  ...
  const conversationId =
    paramId && paramId !== "undefined"
      ? paramId
      : location.pathname.match(/\/supervisor\/messages\/([^/?#]+)/)?.[1] || "";
  ...
  const load = useCallback(async (markRead = false) => {
    ...
    try {
      const res: any = await staffMessageService.thread(conversationId, { limit: 80 });
      if (seq !== loadSeq.current) return;
      setConversation(res?.conversation || null);
      setMessages((res?.rows || []).slice().reverse());
      setLoadError(null);
      if (markRead) staffMessageService.markRead(conversationId).catch(() => {});
      scrollDown();
    } catch (e: any) {
      if (seq !== loadSeq.current) return;
  ...
  const send = async () => {
    ...
    const temp = { id: `tmp_${Date.now()}`, senderType: "staff", senderUserId: myUserId, body, attachments: atts, createdAt: new Date().toISOString(), _pending: true };
    setMessages((m) => [...m, temp]);
    scrollDown();
    try {
      await staffMessageService.send(conversationId, body, newId(), atts);
      await load();
    } catch (e: any) {
  ...
  // Edited image (from the full-screen editor) → upload + send to the thread.
  const sendEditedImage = async (file: File) => {
    if (!validId) return;
    setUploading(true); setSendError(null);
    try {
      const att = await staffMessageService.uploadAttachment(file);
      await staffMessageService.send(conversationId, "", newId(), [att]);
      await load();
    } catch (e: any) {
      fb.error();
      setSendError(e?.message || t("messages.sendFailed", "No se pudo enviar. Reintenta."));
  ...
  const stopRecAndSend = async () => {
    ...
    try {
      ...
      setUploading(true);
      const att = await staffMessageService.uploadAttachment(rec.file);
      fb.press();
      await staffMessageService.send(conversationId, "", newId(), [att]);
      await load();
    } catch (e: any) {
  ...
  return (
    <Screen fill title={title} subtitle={membersLabel} right={<NavActions />}>
      <div className={styles.wrap}>
        <div ref={scrollRef} className={styles.list}>
          ...
          {loading && messages.length === 0 ? (
            ...
            messages.map((m) => {
              const mine = m._pending || (m.senderUserId ? String(m.senderUserId) === myUserId : m.senderType === "staff");
                  ...
                  {mine ? (
                    ...
                    <div className={styles.rowIn}>
                      ...
                      <div className={styles.bubbleIn}>
                        {conversation?.isGroup && m.senderName && <p className={styles.sender} style={{ color: colorFor(senderKey) }}>{m.senderName}</p>}
                        <Attachments m={m} onImageTap={setViewerSrc} />
                        {m.body && <p className={styles.text}>{m.body}</p>}
            ...
            <div className={styles.composer}>
              ...
              {recording ? (
                ...
                <div className={styles.inputWrap}>
                  ...
                  <button onClick={() => { fb.tap(); camRef.current?.click(); }} disabled={uploading} aria-label={t("sos.openCameras", "Cámara")} className={styles.iconBtn}><Camera size={20} /></button>
                </div>
              )}
              {canRecord ? (
